...
class GenerationWorker:
    def __init__(self, args):
        ...
        # Warmup once to reduce the first-request latency spike.
        with self._llm_lock:
            self.llm.generate(
                ["Warm up."],
                self._SamplingParams(temperature=0.8, max_tokens=8, ignore_eos=False),
                use_tqdm=False,
            )
            self._queue_stats_snapshot = self._collect_engine_queue_stats_locked()
